...
const {mabablue, white, lightgrey, black} = colors;
...
export default class DropDown extends PureComponent {
  render = () => {
    const {options = [], toggled = false, history, onAfterClick = () => {}} = this.props;
    const items = options.map(({name, path, displayName}, key) => {
      return (
        <StyledFlex key={key}>
          <MenuItem
            name={name}
